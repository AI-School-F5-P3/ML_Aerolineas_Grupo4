    ...
    if st.session_state.logged_in:
        ...
        if option == "Datos y Predicción":
            ...
            if st.button("Predicciones"):
                ...
                #st.write(f"Predicted class: {np.argmax(st.session_state.rf_probabilities, axis=1)[0]}")
                ...
                nn_probabilities_combined = np.hstack([
                    1 - raw_prediction_nn.reshape(-1, 1),  # Probability of negative class
                        raw_prediction_nn.reshape(-1, 1)        # Probability of positive class
                ])

                st.session_state.nn_probabilities = nn_probabilities_combined
                #st.write("Neural Network :")
                #st.write(f"Predicted probabilities: {st.session_state.nn_probabilities}")
            ...
            if 'prediction_made' not in st.session_state or not st.session_state.prediction_made:
                ...
                with st.form("form_3"):
                    ...
                    # Ensemble Voting: Promedio de las probabilidades
                    average_probabilities = (st.session_state.nn_probabilities + st.session_state.rf_probabilities) / 2
                    final_prediction = classify_value(np.argmax(average_probabilities, axis=1))

                    #Almacenamos la mejor predicción
                    df_to_store['satisfaction'] = final_prediction
