...
def entrenar_y_evaluar_modelo():
    # Asegurar que el directorio de informes para GradientBoosting existe
    dir_informe_gradientboosting = os.path.join(DIR_INFORMES, 'gradientboosting')
    asegurar_directorio(dir_informe_gradientboosting)
    ...
    plt.savefig(os.path.join(dir_informe_gradientboosting, 'matriz_correlacion.png'))
    plt.close()
    ...
    # Entrenar el modelo
    modelo = GradientBoostingClassifier(n_estimators=1000, random_state=42)
    modelo.fit(X_train, y_train)

    # Generar informe detallado
    generar_informe_detallado(modelo, X_train, X_test, y_train, y_test, "GradientBoosting")

    # Guardar el modelo entrenado y el scaler
    asegurar_directorio(DIR_MODELOS)
    joblib.dump(scaler, os.path.join(DIR_MODELOS, 'scaler_gradientboosting1.pkl'))
    joblib.dump(modelo, os.path.join(DIR_MODELOS, 'gradientboosting_model1.pkl'))
...
def cargar_modelo():
    modelo = joblib.load(os.path.join(DIR_MODELOS, 'gradientboosting_model1.pkl'))
    scaler = joblib.load(os.path.join(DIR_MODELOS, 'scaler_gradientboosting1.pkl'))
    return modelo, scaler
