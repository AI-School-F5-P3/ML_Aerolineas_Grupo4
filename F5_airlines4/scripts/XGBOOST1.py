...
def entrenar_y_evaluar_modelo():
    # Asegurar que el directorio de informes para XGBoost existe
    dir_informe_xgboost = os.path.join(DIR_INFORMES, 'xgboost')
    asegurar_directorio(dir_informe_xgboost)
    ...
    plt.savefig(os.path.join(dir_informe_xgboost, 'matriz_correlacion.png'))
    plt.close()
    ...
    # Entrenar el modelo
    modelo = xgb.XGBClassifier(n_estimators=1000, random_state=42, eval_metric='logloss')
    modelo.fit(X_train, y_train)

    # Generar informe detallado
    generar_informe_detallado(modelo, X_train, X_test, y_train, y_test, "XGBoost")

    # Guardar el modelo entrenado y el scaler
    asegurar_directorio(DIR_MODELOS)
    joblib.dump(scaler, os.path.join(DIR_MODELOS, 'scaler_xgboost1.pkl'))
    joblib.dump(modelo, os.path.join(DIR_MODELOS, 'xgboost_model1.pkl'))
...
def cargar_modelo():
    modelo = joblib.load(os.path.join(DIR_MODELOS, 'xgboost_model1.pkl'))
    scaler = joblib.load(os.path.join(DIR_MODELOS, 'scaler_xgboost1.pkl'))
    return modelo, scaler
